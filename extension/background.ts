...
import type { ExtensionMessage, NextStepRequest, NextStepResponse } from '@aws-nav/shared';
...
async function handleNextStepRequest(
  ...
  try {
    console.log('[Background] Proxying next-step request for goal:', payload.goal);
    ...
    console.log('[Background] Received step:', data.step?.instruction?.substring(0, 50));
